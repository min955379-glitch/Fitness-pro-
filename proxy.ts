...
export async function proxy(request:NextRequest){
 ...
 if(request.nextUrl.pathname.startsWith('/admin')&&!user&&!request.nextUrl.pathname.startsWith('/admin/login')) return NextResponse.redirect(new URL('/admin/login',request.url))
 return response
}
